...
// 而关于回文串的题之前也做了不少，处理方法上就是老老实实的两两比较吧。像这种有关极值的问题，最应该优先考虑的就是贪婪算法和动态规划，
...
// 那么其加上2就是dp[i][j]的值。如果s[i] != s[j]，那么我们可以去掉i或j其中的一个字符，然后比较两种情况下所剩的字符串谁dp值大，
// 就赋给dp[i][j]，那么递推公式如下：
...
int main(int argc, char *argv[]) {
  if (Solution::longestPalindromeSubseq("bbbab") != 4) {
    throw std::runtime_error("error 1");
  }
  if (Solution::longestPalindromeSubseq("cbbd") != 2) {
