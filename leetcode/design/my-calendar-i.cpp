...
class MyCalendar {
 public:
  ...
  bool book(int start, int end) {
    auto it = calendar_.lower_bound(start);
    // 1. check if new_start < cur_end
    // 2. check if prev_end > cur_start
    if (it != calendar_.end() && it->first < end) {
      return false;
    } else if (it != calendar_.begin() && std::prev(it)->second > start) {
      return false;
    }
    calendar_[start] = end;
    return true;
