...
// 这样就可以保证有权重的随机啦。那么我们就可以建立权重数组的累加和数组，比如若权重数组为 [1, 3, 2] 的话，
// 那么累加和数组为 [1, 4, 6]，整个的权重和为6，我们 rand() % 6，可以随机出范围 [0, 5] 内的数，随机到 0 则为第一个点，
// 随机到 1，2，3 则为第二个点，随机到 4，5 则为第三个点，所以我们随机出一个数字x后，然后再累加和数组中查找第一个大于随机数x的数字，
// 使用二分查找法可以找到第一个大于随机数x的数字的坐标
class Solution {
 public:
  Solution(std::vector<int> w) : sum_(w) {
  ...
  int pickIndex() {
    int target = std::rand() % sum_.back(), left = 0, right = sum_.size() - 1;
    while (left < right) {
  ...
  int pickIndex_v2() {
    int target = std::rand() % sum_.back();
    return std::upper_bound(sum_.begin(), sum_.end(), target) - sum_.begin();
  }
 private:
  std::vector<int> sum_;
};
...
int main(int argc, char *argv[]) {
  ...
  auto obj1 = new Solution(vec1);
  if (obj1->pickIndex() != 0) {
    throw std::runtime_error("error 1");
  }

  std::vector<int> vec2 = {1, 3};
  auto obj2 = new Solution(vec2);
  if (obj2->pickIndex() != 1) {
