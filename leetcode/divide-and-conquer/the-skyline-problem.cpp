...
class Solution {
 public:
  static std::vector<std::pair<int, int>> getSkyline(std::vector<std::vector<int>> &buildings) {
    std::vector<std::pair<int, int>> points, res;
    ...
    for (auto &point : points) {
      if (point.second < 0) {
        cache.insert(-point.second);
      } else {
        cache.erase(cache.find(point.second));
      }
      cur = *cache.rbegin();
      if (cur != pre) {
...
int main(int argc, char *argv[]) {
  std::vector<std::vector<int>> vec = {{2, 9, 10}, {3, 7, 15}, {5, 12, 12}, {15, 20, 10}, {19, 24, 8}};
  std::vector<std::pair<int, int>> out = {{2, 10}, {3, 15}, {7, 12}, {12, 0}, {15, 10}, {20, 8}, {24, 0}};

  if (Solution::getSkyline(vec) != out) {
